...
import ColdCard from "../../components/cards/ColdCard";
import ColdTable from "../../components/table/ColdTable";
...
const Cold = ({ setLeadVal, leads, refetch }) => {
  ...
  const exportLeads = async () => {
    try {
      ...
      const response = await fetch("http://localhost:8000/auth/export/coldleads", {
      ...
      a.href = url;
      a.download = "coldleads.csv"; // Set the filename for the downloaded file
          ...
          <div className="flex gap-3 xl:gap-5">
            <div className="flex gap-2 bg-white drop-shadow-md rounded-md px-2 lg:px-3 py-2 text-sm items-center justify-center">
              ...
              <input
                ...
                className="sm:block  outline-none border-none w-40"
              />
            </div>
            <DropDown
              option={sortOptions}
        ...
        {cardview ? (
          <div className="h-full py-5 overflow-auto">
            {filteredLeads.length > 0 ? (
              <ColdTable leads={filteredLeads} refetch={() => refetch()} />
            ) : (
              ""
            )}
          </div>
          ...
          <div className=" py-5 gap-2 md:gap-4 grid grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(200px,1fr))] lg:grid-cols-[repeat(auto-fill,minmax(230px,1fr))] overflow-y-auto">
            {filteredLeads.map((lead) => (
              <ColdCard key={lead._id} lead={lead} refetch={() => refetch()} />
            ))}
          </div>
        )}
      </div>
